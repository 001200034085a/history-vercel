...
export default function AdminItem({_id,name,image,dob,pob,description,address,country,date,career,prize,evaluate,period,medal,position,honnors,url,imageVideo,likeVideo,dislikeVideo,like, dislike, star,data,setData,onEditItem}){
    ...
    let result = await fetch(`https://history-api-production.up.railway.app/api/history/${_id}`,{
      method:"put",
    ...
    return(
        ...
        <div>
            <List.Item
                    ...
                    <DeleteOutlined onClick={(event)=>onDeleteItem(_id)} />
                    {editing? <SaveOutlined onClick={edit} />:<EditOutlined onClick={anebleEdit}/>} 
                    </List.Item>
        </div>
    )
}